...
export const projects: Project[] = [
  ...
  {
    title: 'Waresport Intern Portal',
    category: 'Fullstack',
    description: 'Production-ready full-stack intern management portal built for Waresport, featuring role-based auth, task management, weekly scrum logs, a knowledge base, and an admin dashboard — deployed on Vercel with a PostgreSQL backend.',
    problem: 'Managing a cohort of interns across tracks requires coordinating task assignments, tracking progress, collecting weekly reflections, and surfacing completion metrics — all without a dedicated internal tool.',
    approach: 'Built with Next.js 14 App Router, NextAuth.js v4 (credentials provider with first-login password change flow), Prisma ORM against PostgreSQL (Supabase/Railway), and Tailwind CSS with custom shadcn-inspired components. Implemented role-based routing (Admin/Intern), task assignment by track or individual, weekly scrum log submissions with Friday reminders via SMTP, a required-reading knowledge base, and an announcements system. Admin views include completion rate dashboards and a full intern roster with per-intern drill-down.',
    outcome: 'Deployed to production on Vercel — gives Waresport admins a single place to assign work, track intern progress, and collect weekly reflections, while interns get a clean week-at-a-glance dashboard for their tasks and logs.',
    techStack: ['Next.js 14', 'TypeScript', 'NextAuth.js', 'Prisma', 'PostgreSQL', 'Supabase', 'Tailwind CSS', 'Vercel', 'SMTP', 'App Router'],
    githubUrl: 'https://github.com/vidyutraj/waresport-summer-internship',
    demoUrl: 'https://waresport-summer-internship.vercel.app/',
    achievements: [
      'Role-based auth (Admin / Intern) with protected routes and first-login password change flow',
      'Task assignment by track, all interns, or individual — with per-intern completion tracking',
      'Weekly scrum log submissions with automated Friday reminder emails via SMTP',
      'Admin dashboard with completion rates, intern roster, and per-intern drill-down views',
      'Deployed to production on Vercel with Supabase PostgreSQL backend',
    ],
    additionalDetails: 'Schema covers User (ADMIN | INTERN roles, track, profile), Task (week number, assignedTo: ALL | TRACK | INDIVIDUAL), TaskAssignment (per-intern status with completedAt timestamp), WeeklyLog (scrum-style reflection), Resource (knowledge base with categories and required flag), and Announcement (all-intern broadcast). Intern routes: /dashboard, /tasks, /tasks/[id], /logs, /resources, /profile. Admin routes: /admin, /admin/interns, /admin/interns/[id], /admin/tasks, /admin/logs, /admin/resources, /admin/announcements.',
  },
  {
    title: 'Harmonized Controls Generator',
    category: 'Cybersecurity Labs',
    description: 'Full-stack compliance harmonization platform that ingests ISO 27001, ISO 27701, and HITRUST controls, uses AI embeddings and clustering to eliminate duplicate controls, and produces a unified control set with full traceability back to original framework IDs.',
    problem: 'Organizations subject to multiple compliance frameworks (ISO 27001, ISO 27701, HITRUST) face massive control overlap — manually reconciling hundreds of redundant controls is time-consuming and error-prone.',
    approach: 'Built a Python/Flask pipeline with three stages: (1) load and normalize controls from XLSX/CSV sources, splitting long descriptions into sub-controls via sentence tokenization; (2) generate semantic embeddings with sentence-transformers and cluster similar controls using Agglomerative Clustering; (3) use GPT-4o-mini to synthesize each cluster into a single harmonized control with auto-categorization. A React-style web UI provides real-time pipeline progress, KPI cards, interactive Chart.js visualizations (doughnut, bar, line), framework overlap analysis, table/card views with full-text search, clickable original-control mappings, and CSV export.',
    outcome: 'Reduces total control count by eliminating cross-framework duplicates while maintaining 100% coverage — every original control ID is traceable to a harmonized control. Outputs a structured CSV with cluster IDs, categories, harmonized text, and semicolon-separated source mappings ready for audit.',
    techStack: ['Python', 'Flask', 'OpenAI GPT-4o-mini', 'Sentence Transformers', 'Agglomerative Clustering', 'Pandas', 'Chart.js', 'HTML', 'CSS', 'JavaScript', 'ISO 27001', 'ISO 27701', 'HITRUST'],
    githubUrl: 'https://github.com/vidyutraj/Cybercompliance-Automation',
    achievements: [
      'Harmonizes controls across ISO 27001, ISO 27701, and HITRUST in a single pipeline run',
      'Semantic deduplication via sentence-transformer embeddings + agglomerative clustering',
      'GPT-4o-mini synthesizes each cluster into a unified, categorized control statement',
      '100% coverage guarantee — all original control IDs mapped and traceable for audit',
      'Interactive dashboard with KPIs, framework overlap analysis, and CSV export',
    ],
    additionalDetails: 'Pipeline: Step 1 (pipeline.py) loads and normalizes all three frameworks, splits long descriptions into sub-controls. Step 2 (cluster_controls.py) generates semantic embeddings and assigns cluster IDs. Step 3 (embedding_pipeline.py) groups by cluster, calls GPT-4o-mini to write harmonized control text, and outputs harmonized_controls_final.csv with ClusterID, Category, HarmonizedControl, Mappings, and NumSubControls columns. The Flask app (app.py) exposes REST endpoints for file upload, pipeline execution, controls retrieval, original-control lookup, and CSV download, with real-time progress streamed to the UI.',
  },
  {
    title: 'WhenAmIFree',
    category: 'Automation & Tooling',
    description: 'Chrome Extension that connects to Google Calendar and drafts ready-to-send availability messages for recruiters — pick a date range, set your hours and slot duration, and get a formatted message in seconds.',
    problem: 'Manually scanning your calendar and writing out availability windows for recruiters is tedious and error-prone.',
    approach: 'Built a Manifest V3 Chrome Extension using chrome.identity for OAuth token management and the Google Calendar freeBusy API to find open windows. The UI lets you set a date range (with quick-select chips), working hours, slot duration (30/45/60/90 min or custom), days of week, timezone, and message style (Professional, Casual, or Slots only). A local test harness (test.html) supports both mock mode and real calendar mode for validating free-window logic without loading the extension.',
    outcome: 'Generates a copy-ready availability message from your live calendar in seconds, eliminating manual scheduling back-and-forth with recruiters.',
    techStack: ['Chrome Extension', 'Manifest V3', 'Google Calendar API', 'Google OAuth', 'chrome.identity', 'Vanilla JS', 'HTML', 'CSS'],
    githubUrl: 'https://github.com/vidyutraj/whenamifree',
    achievements: [
      'Zero-dependency — vanilla JS, no frameworks or build step required',
      'Supports Professional, Casual, and Slots-only message styles',
      'Auto-detects browser timezone with manual override',
